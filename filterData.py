...
parser.add_argument('-i', action="store", dest="inputFolder")
parser.add_argument('-f', action="store", dest="filterLength", type=float)
...
parser.add_argument('--input', action="store", dest="inputFolder")
...
M_PI = math.pi
ft1 = firstCutOff / samplingFreq
ft2 = secondCutOff / samplingFreq

for sample in xrange(0, filterLength):
    if (sample != M / 2)
